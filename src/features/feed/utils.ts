...
export const getUserBadges = (user: User) => {
  ...
  if (topVerifier && user.reportsVerified && user.reportsVerified >= 10) {
  ...
  if (probSolver && user.reportsResolved && user.reportsResolved >= 1) {
    badges.push(probSolver);
  }
  
  const civicHero = ALL_BADGES.find(b => b.id === 'civic_hero');
  if (civicHero && user.reputationScore && user.reputationScore >= 600) {
