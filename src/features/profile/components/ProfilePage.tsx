...
import { User, Community, Issue, Comment, UserActivity, FeedPost } from '../../../types';
import { getLevelInfo, getUserBadges } from '../../feed/utils';
import { PRESET_COVERS } from '../../../lib/constants';
...
export default function ProfilePage({
  ...
  const mySupportedIssues = issues.filter(issue => {
    const matchingPostId = `post_${issue.id}`;
    return supportedPostIds.includes(matchingPostId) || supportedPostIds.includes(issue.id);
  });
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative">
          ...
          <AnimatePresence>
            {showCoverPicker && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="absolute top-14 right-4 bg-slate-900/95 backdrop-blur-md p-3 rounded-2xl shadow-xl z-20 border border-slate-800 flex gap-2"
              >
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end gap-4 text-center md:text-left">
            <div className="relative">
              <img 
                ...
                alt={user.name} 
                className="w-28 h-28 md:w-36 md:h-36 rounded-3xl object-cover border-4 border-white shadow-xl bg-slate-50"
              />
            ...
            <div className="mb-2">
              <div className="flex items-center justify-center md:justify-start gap-2 flex-wrap">
                ...
                <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${levelInfo.color}`}>
              ...
              <div className="flex items-center justify-center md:justify-start gap-4 mt-2 text-xs text-slate-500 font-medium">
                {user.location && (
          ...
          <div className="flex justify-center self-center md:self-end">
            {isEditing ? (
              ...
              <button
                ...
                className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black rounded-xl shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
              >
        ...
        {isEditing && (
          <div className="px-6 md:px-8 pb-6 border-t border-slate-100 pt-6 bg-slate-50/50 space-y-4">
            <h3 className="text-xs font-black text-slate-900 uppercase tracking-wider">Edit Public Info</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-3 py-2 bg-white border border-slate-200 text-slate-800 text-xs rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                />
              </div>
              <div>
        ...
        {!isEditing && user.bio && (
          <div className="px-6 md:px-8 pb-6 pt-2 border-t border-slate-100/50">
            <p className="text-xs text-slate-600 leading-relaxed italic max-w-2xl bg-slate-50/50 px-4 py-3 rounded-2xl border border-slate-100">
              "{user.bio}"
            </p>
          </div>
        )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-2">
          ...
          <span className="block text-2xl font-black text-slate-900 font-mono">{user.reputationScore} XP</span>
          <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
            <div className="bg-orange-500 h-full rounded-full" style={{ width: `${Math.min(100, (user.reputationScore % 300) / 3)}%` }} />
          </div>
          <span className="text-[9px] font-bold text-slate-400 block font-mono">Next level at {Math.ceil((user.reputationScore + 1) / 300) * 300} XP</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-2">
          ...
          <span className="block text-2xl font-black text-emerald-600 font-mono">{user.trustScore || 95}%</span>
          <span className="text-[9px] font-bold text-slate-400 block">Calculated via valid audit logs & resolves</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-2">
          ...
          <span className="block text-2xl font-black text-slate-900 font-mono">{user.reportsCreated}</span>
          <span className="text-[9px] font-bold text-slate-400 block">Civic hazards submitted</span>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-2">
          ...
          <span className="block text-2xl font-black text-slate-900 font-mono">{user.reportsResolved}</span>
          <span className="text-[9px] font-bold text-slate-400 block">Verified resolutions completed</span>
        </div>
      </div>

      {/* Tabs list navigation */}
      <div className="flex bg-slate-105 p-1.5 rounded-2xl overflow-x-auto whitespace-nowrap scrollbar-none">
        {(['activity', 'reports', 'communities', 'supported', 'comments', 'badges', 'saved'] as const).map((tab) => (
          <button
            key={tab}
