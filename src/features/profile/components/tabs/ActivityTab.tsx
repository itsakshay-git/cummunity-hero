...
export const ActivityTab: React.FC<ActivityTabProps> = ({ activities }) => {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-black text-slate-900 dark:text-slate-100">Activity Ledger</h3>
      <div className="relative border-l border-slate-100 dark:border-slate-800 pl-4 space-y-6">
        {activities.map(act => (
          <div key={act.id} className="relative">
            {/* Timeline dot */}
            <span className="absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full bg-emerald-500 border border-white dark:border-slate-900" />
            <div className="space-y-0.5">
              ...
              <div className="flex items-center gap-3 text-[10px] font-mono text-slate-450 dark:text-slate-500">
                ...
                {act.pointsEarned > 0 && (
                  <span className="text-emerald-600 dark:text-emerald-500 font-bold">+{act.pointsEarned} XP earned</span>
                )}
              </div>
            </div>
          </div>
        ...
        {activities.length === 0 && (
          <p className="text-xs text-slate-500 dark:text-slate-450 italic">No activities logged yet.</p>
        )}
      </div>
    </div>
  );
